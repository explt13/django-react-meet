    ...
    return (
        <>
            {resultList.map(user => (
                ...
                >
                    <div className={[classes.userImage, props.forMap ? classes.forMap : undefined].join(' ')}>
                        <img src={`http://127.0.0.1:8000/${user.profile_pic}`} />
                    </div>
                    <div className={[classes.information, props.forMap ? classes.forMap : undefined].join(' ')}>
                        <div className={[classes.userFullName, props.forMap ? classes.forMap : undefined].join(' ')}>
                            {!props.forMap
                            ? <Link to={`/user/${user.username}`}>{user.first_name} {user.last_name}</Link>
                            : user.first_name
                            }
                        </div>
                        <div className={[classes.userUsername, props.forMap ? classes.forMap : undefined].join(' ')}>
                            {user.username}
                        </div>
